...
def detect_default_workspace(root: Path) -> Path | None:
  if root.as_posix().startswith("/mnt/"):
    return Path("/tmp/little-anti-cheat-build")
  return None
...
def main() -> int:
  parser = argparse.ArgumentParser(description="Compile Little Anti-Cheat SourceMod plugins into a local build directory.")
